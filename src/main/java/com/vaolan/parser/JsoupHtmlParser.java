...
import com.vaolan.utils.StringOperatorUtil;
import com.vaolan.utils.SystemAssert;
import com.vaolan.utils.UrlOperatorUtil;
...
public class JsoupHtmlParser {
	...
	public static String removeInnerContent(String htmlSource, String selector,
		...
		try {
			...
			String result = null;
			if (elements != null) {
				if (StringOperatorUtil.isNotBlankCollection(removeSelector)) {
					for (Element ele : elements) {
					...
					result = JsoupHtmlParser.getCleanTxt(elements.get(0)
							.toString());
				}
			}
			return result;
